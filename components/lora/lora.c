 ...
 * Dio0Mapping values
 */
#define MODE_DIO0_RXDONE               0x00
#define MODE_DIO0_TXDONE               0x40
...
static uint8_t buffer_reception[256] = {0};
...
/**
 * Write a value to a register from an ISR.
 * @param reg Register index.
 * @param val Value to write.
 */
void
lora_write_reg_from_isr(int reg, int val)
{
	uint8_t out[2] = { 0x80 | reg, val };
	uint8_t in[2];
...
/**
 * Read the current value of a register from an ISR.
 * @param reg Register index.
 * @return Value of the register.
 */
int
lora_read_reg_from_isr(int reg)
{
	uint8_t out[2] = { reg, 0xff };
	uint8_t in[2];
...
/**
 * Sets the radio transceiver in idle mode from an ISR.
 * Must be used to change registers and access the FIFO.
 */
void
lora_idle_from_isr(void)
{
	lora_write_reg_from_isr(REG_OP_MODE, MODE_LONG_RANGE_MODE | MODE_STDBY);
}
...
/**
 * Sets the radio transceiver in receive mode from an ISR.
 * Incoming packets will be received.
 */
void
lora_receive_from_isr(void)
{
	lora_write_reg_from_isr(REG_OP_MODE, MODE_LONG_RANGE_MODE | MODE_RX_CONTINUOUS);
}
...
/**
 * Send a packet enabling TX done IRQ.
 * @param buf Data to be sent
 * @param size Size of data.
 */
void 
lora_send_packet_with_tx_done(uint8_t *buf, int size)
{
	/*
	...
	/* Enable TX done IRQ */
	lora_write_reg(REG_DIO_MAPPING_1, MODE_DIO0_TXDONE);
...
/**
 * Send a packet and waits until done transmission.
 * @param buf Data to be sent
 * @param size Size of data.
 */
void
...
static void IRAM_ATTR lora_handleDio0Rise(void* arg)
{
	int irqFlags;
	int packetLength;
	irqFlags = lora_read_reg_from_isr(REG_IRQ_FLAGS);

	// clear IRQ's
	lora_write_reg_from_isr(REG_IRQ_FLAGS, irqFlags);
	...
	if ((irqFlags & IRQ_PAYLOAD_CRC_ERROR_MASK) == 0) {
		...
		if ((irqFlags & IRQ_RX_DONE_MASK) != 0) {
			...
			// read packet length
			packetLength = __implicit ? lora_read_reg_from_isr(REG_PAYLOAD_LENGTH) : lora_read_reg_from_isr(REG_RX_NB_BYTES);

			// set FIFO address to current RX address
			lora_write_reg_from_isr(REG_FIFO_ADDR_PTR, lora_read_reg_from_isr(REG_FIFO_RX_CURRENT_ADDR));
			...
			if (_onReceive) {
				...
				for(int i=0; i < packetLength; i++)
					buffer_reception[i] = lora_read_reg_from_isr(REG_FIFO);
				_onReceive(buffer_reception, packetLength);
			}
		}
			...
			lora_write_reg_from_isr(REG_DIO_MAPPING_1, MODE_DIO0_RXDONE);
		}
	}
}
...
void lora_onReceive(void(*callback)(uint8_t*, int))
{
	if (callback) {
		_onReceive = callback;
		if (_onTxDone) {
			/* gpio isr already registered, do nothing */
		}
		else {
			...
			io_conf.pin_bit_mask = (1ULL<<CONFIG_DIO_GPIO);
			//set as input mode
			...
			gpio_isr_handler_add(CONFIG_DIO_GPIO, lora_handleDio0Rise, (void*) CONFIG_DIO_GPIO);
		}
	}
	else {
		if ( _onReceive || _onTxDone) {
			//remove isr handler for gpio number.
			gpio_isr_handler_remove(CONFIG_DIO_GPIO);
...
void lora_onTxDone(void(*callback)())
{
	if (callback) {
		...
		if (_onReceive) {
			...
			//hook isr handler for specific gpio pin
			gpio_isr_handler_add(CONFIG_DIO_GPIO, lora_handleDio0Rise, (void*) CONFIG_DIO_GPIO);
			ets_printf("registered gpio dio isr\n");
		}
	}
	else {
		if ( _onReceive || _onTxDone) {
